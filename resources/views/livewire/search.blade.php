<form class="mt-4 pt-2 relative mx-auto text-gray-600" autocomplete="off">
    ...
    @if ($search)
        ...
        <ul class="absolute z-50 left-0 w-full bg-white mt-1 rounded-lg overflow-hidden">
            @forelse ($this->results as $result)
                <li class="leading-10 text-sm px-5 cursor-pointer hover:bg-gray-300">
                    <a href="{{ route('courses.show', $result) }}">
                        {{ $result->title }}
                    </a>
                </li>
            @empty
                <li class="leading-10 text-sm px-5 hover:bg-gray-300">
